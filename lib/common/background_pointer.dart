...
import 'package:display_speed_project/common/cloud_data.dart';
...
class CloudBackgroundPainter extends CustomPainter{
    ...
    for(int i = 0; i < 75; i++){
      ...
      for(int i = 0; i < 4 + rnd.nextInt(2); i++){
        path.addOval(Rect.fromCircle(
          center: Offset(rnd.nextInt(30) - 15, rnd.nextInt(16) - 8),
          radius: 10.0 + rnd.nextInt(10)
        ));
      }
  ...
  Path getCloudPath(Size size){
    ...
    path.addOval(Rect.fromLTWH(10, -7, width / 4, height / 4));
    path.addOval(Rect.fromLTWH(0, 5, width / 4, height / 4.5));
    return path;
  }
